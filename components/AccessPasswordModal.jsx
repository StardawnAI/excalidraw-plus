  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      if (data.valid) {
        ...
        setMessage(data.message || 'Password validation failed');
        setMessageType('error');
      }
    } catch (error) {
      ...
      <div className="relative bg-white rounded border border-gray-300 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div className="px-4 py-3 bg-blue-50 border border-blue-200 rounded">
            ...
            <p className="text-sm text-blue-800">• Access password has higher priority than frontend LLM configuration</p>
            <p className="text-sm text-blue-800">• When enabled, server-side configured LLM will be used</p>
          </div>
