/**
 * Configuration Manager - Handles multiple LLM API configurations
 */
...
class ConfigManager {
  ...
  validateConfig(config) {
    ...
    if (!config.type || !['openai', 'anthropic'].includes(config.type)) {
      errors.push('Configuration type must be openai or anthropic');
    }
    ...
    if (!config.baseUrl || config.baseUrl.trim() === '') {
      ...
      try {
        ...
        errors.push('API URL format is invalid');
      }
    }
